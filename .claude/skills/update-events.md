...
- **The last `<h3>` label on a page used to swallow the article body (fixed 2026-08-30).** `H3_BLOCK_RE` in `fetch_conservancy_events.py` captured from a label until the next `<h3>`/`</section>`/`</article>`/`<footer>`. Because the labels sit in a `<dt><h3>Label</h3></dt><dd>value</dd>` list and "Location" is usually last, nothing stopped the capture at the end of its `<dd>` — so `location_detail` came back as the venue name with the entire event description run on behind it (1,239 chars for the vision-boarding workshop, 1,716 for Founders' Day; nine cached records over 120 chars). `place` still resolved, because `match_places()` found the venue inside the blob, but the raw string landed in the event's `location:` front matter and rendered as a paragraph where the location line belongs. The regex now also terminates at `</dd>` and `</dl>`. **If you touch this regex, re-check the venue length** — a silently over-long `location_detail` looks fine in the cache and only shows up on the rendered page.
